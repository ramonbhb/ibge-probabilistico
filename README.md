...
Treino e aplicação são separados: o modelo é treinado no limpo (`02_treinar`). A aplicação (`02b_aplicar`) carrega o JSON e aplica as 11 regras de predição no limpo **sem** a lista de ouro. Diagnóstico dos pares no [`03_avaliar.ipynb`](notebooks/03_avaliar.ipynb). Lista operacional no [`04_atribuir.ipynb`](notebooks/04_atribuir.ipynb) (até 3 Censos por CPF). Corte: **`THRESHOLD_AVALIACAO`** (default 0,99). O [`05_adicionar_regras.ipynb`](notebooks/05_adicionar_regras.ipynb) acrescenta pares na faixa `[0,95, T)` em funil: `nome_mae_phon` (igual ou prefixo de `n ≥ 2` tokens); senão nome completo + CEP; senão pontas + CEP. No 04, par com mães preenchidas e incompatíveis não compete, salvo nome fonético e DOB iguais.

## Pré-requisitos

1. Parquets bronze; `cohort_dedup.parquet` só para **carimbar** `cpf_norm` no Censo no treino (`COHORT_DEDUP_ARQUIVO`); lista de ouro em `LISTA_OURO_ARQUIVO` para o 00b (tirar da aplicação) e o 03b (validar)
...
| [`03_avaliar.ipynb`](notebooks/03_avaliar.ipynb) | Funil do Censo de aplicação, exemplos ≥ T e faixa, melhor CPF, discordância nome/DOB, 1:1 abaixo de T |
...
**Pipeline:** `00` → `00b` → `01` → `02` treinar → `02b` aplicar → `03` diagnosticar aplicação → `04` exportar lista → `05` regras na faixa. Validação da ouro: `03b` (limpo cheio + `LISTA_OURO_ARQUIVO`; não usa o parquet do 02b).
...
**1ª passada do modelo (`02_treinar`):** comparisons, prior e EM estão nas células do [`02_treinar_splink.ipynb`](notebooks/02_treinar_splink.ipynb). O [`02b_aplicar_splink.ipynb`](notebooks/02b_aplicar_splink.ipynb) carrega o JSON e aplica as 11 regras de predição (não estão no treino). Sem `nome_mae*` (Censo ~34% preenchido no recorte MA; exact dava peso demais). Nomes no score: `nome_completo_phon` (Null → Exact+TF → prefixo de tokens `min(n) ≥ 2` → último token igual e JW ≥ 0,95 / 0,92 → ELSE; JW só se o último coincidir — `RODRIGUES` vs `PEREIRA` cai no ELSE), `primeiro_nome_phon` e `ultimo_nome_phon` (exact + Jaro-Winkler 0,95–0,92 + TF). Meio e o composto `primeiro_ultimo*` ficam nas tabelas; **não** entram no comparison. Primeiro e último também entram no blocking de predição. DOB: uma comparison (`data_nascimento`) com Null custom → Exact ISO + TF → Damerau ≤ 1 → Damerau ≤ 2 → ELSE `m=1e-6` fixo. Damerau na string ISO pega transposições `01`↔`10` e o dia `08` vs `10` (distância 2). `|Δano|` grande (ex. 10 anos, mesmo aniversário) cai no ELSE e pune; `1999` vs `2000` no ano também (Damerau 4). Não usamos `DateOfBirthComparison` (diffs de mês/ano em época). Idade: exact e `abs(diff) ≤ 1`; ELSE `m=1e-6` fixo (só pesa quando a idade do Censo não é nula). UF no score (`ExactMatch` + TF). **Sexo não entra na nota.** CEP não entra no score; entra em três regras de predição (`ultimo+mes+dia+sexo+cep`, `ultimo+mes+ano+sexo+cep`, `DOB+sexo+cep`) e no EM. Município (`cod_municipio`) nas duas regras de primeiro nome (mês+dia e mês+ano). **`cpf_norm`:** no CPF vem do bronze; no Censo, da `cohort_dedup` (`PERSON_ID_CENSO` → `CPF_NORM`, `MIN` se ambíguo; NULL fora da coorte). Entra no prior determinístico e num EM; **não** entra no blocking de predição nem no score — senão a ouro sempre seria candidata e o 03b circularia. `NULL = NULL` é falso. Mãe fica para uma 2ª passada. Depois do treino, o `02` grava `splink_model.json` em `OUTPUT_DIR` e uma cópia em [`models/splink_model.json`](models/splink_model.json). JSON antigo com `bayes_factor_column_prefix` (`bf_`) é do contrato anterior — retreinar no `02_treinar`. Depois desta mudança de comparação/blocking, **retreinar**: m/u mudam e o JSON em `models/` continua inválido até esse treino.
...
**Blocking Splink:** três listas distintas — predição (11 regras OR, recall) no [`02b_aplicar_splink.ipynb`](notebooks/02b_aplicar_splink.ipynb); prior (`nome_completo+DOB` e `cpf_norm`) e EM (quatro blocos apertados para `m`) no [`02_treinar_splink.ipynb`](notebooks/02_treinar_splink.ipynb). Não unificar. Fonte das 11 regras: célula `blocking_rules` do 02b. `cpf_norm` e `nome_meio` **não** entram na predição. Partes da data vêm da view `splink_input` (`ano_nascimento` / `mes_nascimento` / `dia_nascimento`, `substr` da ISO). Sexo em `ultimo+mes+dia+sexo+cep`, `ultimo+mes+ano+sexo+cep` e `DOB+sexo+cep`. CEP nessas três; município nas duas de primeiro nome.
...
- `primeiro_nome_phon` + `mes_nascimento` + `dia_nascimento` + `cod_municipio`
...
- **`cpf_norm`:** no CPF, `COD_CPF` normalizado (11 díg.). No Censo, join com `cohort_dedup` (`PERSON_ID_CENSO` → `CPF_NORM`; `MIN` se ambíguo; NULL fora da coorte). Coluna estrutural. Treino: prior + EM. Não entra no score nem nas 11 regras de predição.
...
| `splink_predictions_ouro.parquet` | `03b` (Censo ouro × `cpf_limpo`) |
...
A lista de ouro (`LISTA_OURO_ARQUIVO`, default `regra1.parquet`) é o rótulo de validação. O 00b tira esses pares da **aplicação**. O [`03b_avaliar_lista_ouro.ipynb`](notebooks/03b_avaliar_lista_ouro.ipynb) roda o modelo nos Censos da lista contra `cpf_limpo` (estado, CPFs da ouro **dentro**) e mede precisão/recall/FP — ver [`README_validacao_ouro.md`](README_validacao_ouro.md).

`cohort_dedup.parquet` **não** entra na avaliação. Continua só para **carimbar** `cpf_norm` no Censo (NB00/00b) no treino (prior/EM). O score e o blocking de predição **não** vêem o CPF.

- **03:** exemplos ≥ T e na faixa; melhor CPF por Censo; funil do Censo de aplicação; discordância nome/DOB; 1:1 com `p < T`.
- **03b:** lista de ouro 1:1; associação certa/errada; FN decomposto; FP da escolha.
- **04:** exporta só as associações únicas (mesmo SQL do 03/03b).
