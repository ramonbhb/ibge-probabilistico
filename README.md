...
No Censo a idade vem de **`PECP0401`** (variável auxiliar calculada, 0–140 anos, universo) no NB00. No NB00b, se a data validada não é nula, a idade do Censo vira `NULL` — a comparison de idade no Splink cai no NullLevel e não duplica a DOB. Sem data, `PECP0401` permanece. No CPF a idade é derivada de `data_nascimento` como anos completos em **`DATA_REFERENCIA_IDADE`** (`2022-08-01`), via `age()` no DuckDB — não é só `2022 - ano`. No Splink a comparação usa exact e `abs(idade_l - idade_r) ≤ 1` (não colunas `idade±1`). As colunas do questionário (`PECP0003`/`PECP0030`) ficam só como contexto no diagnóstico do NB00.
